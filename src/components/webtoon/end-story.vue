  ...
  <view :style="{ flex: 1,marginTop:20 }">
    <image-background :style="{flex: 1}" resizeMode="cover" :source="backgroundImage">      
      ...
      <view :style="{marginTop:20}">
        ...
        <view :style="{marginTop: 30, marginBottom: 30}">
          ...
          <view :style="{margin: 12}">
            <next-chapter-button v-if="nextStory" 
            :data="nextStory"            
             />
          </view>
          <!-- Button read -->
      ...
      <view
        ...
        <stamp :handleOverlayClick="handleOverlayClick" :showStampInput="showStampInput" :style="{padding: 20}" :displaySticker="saveStamp" />
      </view>
      <!--/ stamp input -->
    ...
    <view class="background-modal" :style="{ flex:1, justifyContent: 'flex-end', alignItems:'center'}">      
      <!-- <view v-if="showCommentInput" :style="{height:showCommentInput?400:50,  zIndex:11}"> -->
      <TouchableWithoutFeedback>      
      <keyboard-avoiding-view
        v-if="showCommentInput"
        keyboardVerticalOffset=100
        :style="[kb_AvoidViewStyle, {  zIndex: 11,   }]"
        behavior="padding"
      ...
      >
        <comment-input
          v-if="showCommentInput"
          :style="{ height: 50, width: '95%',position:'absolute', top:-50}"
...
export default {
  props: [
    'navigation', 'relatedBooks', 'getNextChapter', 'book', 'reloadStory', 'story'
  ],
  components: {
    HeaderImage,
    backgroundImage,
  ...
  async created(){            
    let nextChapterData = this.getNextChapter          
